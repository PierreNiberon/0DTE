...
def analyze_single_day_puts():
    """Analyze put options pricing structure for a single day"""
    ...
    # Filter data for the selected date and puts only
    day_data = df[(df['date'] == selected_date) & (df['option_type'] == 'put')].copy()
    ...
    print(f"Number of put options: {len(day_data)}")
    
    # Calculate moneyness and other metrics
    day_data['moneyness'] = (day_data['strike'] - spx_price) / spx_price
    day_data['intrinsic_value'] = np.maximum(day_data['strike'] - spx_price, 0)
    ...
    ax1.plot(day_data['strike'], day_data['lastPrice'], 'ro-', linewidth=2, markersize=4)
    ax1.axvline(x=spx_price, color='blue', linestyle='--', linewidth=2, label=f'SPX Close: ${spx_price:.0f}')
    ax1.set_xlabel('Strike Price ($)')
    ax1.set_ylabel('Put Option Price ($)')
    ax1.set_title(f'Put Option Prices vs Strike Price\n{selected_date.strftime("%Y-%m-%d")}')
    ...
    ax2.axvline(x=spx_price, color='blue', linestyle='--', linewidth=2, label=f'SPX Close')
    ...
    ax3.plot(day_data['moneyness'], day_data['impliedVolatility'], 'mo-', linewidth=2, markersize=4)
    ax3.axvline(x=0, color='blue', linestyle='--', linewidth=2, label='ATM')
    ...
    # 4. Volume vs Strike
    ax4.bar(day_data['strike'], day_data['volume'], width=width*0.6, alpha=0.7, color='darkred')
    ax4.axvline(x=spx_price, color='blue', linestyle='--', linewidth=2, label=f'SPX Close')
...
SPX Close: ${spx_price:.2f}  |  Put Options: {len(day_data)}  |  Strike Range: ${day_data['strike'].min():.0f}-${day_data['strike'].max():.0f}
ATM Options: {len(day_data[abs(day_data['moneyness']) < 0.01])}  |  ITM: {len(day_data[day_data['moneyness'] > 0])}  |  OTM: {len(day_data[day_data['moneyness'] < 0])}''', 
             fontsize=10, bbox=dict(boxstyle='round', facecolor='lightpink', alpha=0.8))
    plt.savefig('visualizations/05_single_day_puts_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"\nPut options analysis saved as 'visualizations/05_single_day_puts_analysis.png'")
    
    # Print detailed analysis
    print(f"\n=== PUT OPTIONS PRICING ANALYSIS ===")
    print(f"Date: {selected_date.strftime('%Y-%m-%d')}")
    print(f"SPX Close: ${spx_price:.2f}")
    print(f"Total Put Options: {len(day_data)}")
    ...
    # Price breakdown (note: for puts, ITM is strike > SPX, OTM is strike < SPX)
    itm_count = len(day_data[day_data['moneyness'] > 0])  # Puts are ITM when strike > spot
    otm_count = len(day_data[day_data['moneyness'] < 0])  # Puts are OTM when strike < spot
    print(f"\nMoneyness Breakdown:")
